...
import { faFileExcel,faAddressCard } from '@fortawesome/free-solid-svg-icons';
...
export class TrabOpcionesComponent implements OnInit {
  ...
  public tblData(){
   ...
   public action(optDes: any , optLink :any ,optSub : any , tipo :string ) : boolean{
    let url      = '';
    ...
    if(tipo =='up'){
      url      = 'updOpt';
      des      = 'Opción ' + optDes + ' fue actualizada.';
      idEtaDes = 7;
      lgDes    = 'ACUALIZAR ROL';
    }else{
      url      = 'insOpt';
      des      = 'Opción ' + optDes + ' fue ingresada.';
      idEtaDes = 6;
      lgDes    = 'INGRESO DE ROL';
    }
    let opciones = {'optDes' : optDes , 'optLink': optLink , 'optSub':optSub }
    
    this.serviLoad.sumar.emit(2);
   this.rest.post(url, this.token, opciones).subscribe(resp => {
