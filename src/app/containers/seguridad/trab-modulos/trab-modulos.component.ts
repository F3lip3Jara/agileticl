...
export class TrabModulosComponent implements OnInit {
  ...
  public tblData(){
   ...
   public action(molDesx : any , molIcon : any , tipo :string ) : boolean{
    let url      = '';
    ...
    if(tipo =='up'){
      url      = 'updMol';
      des      = 'MÓDULO ' + molDesx + ' fue actualizado.';
      idEtaDes = 7;
      lgDes    = 'ACUALIZAR MÓDULO';
    }else{
      url      = 'insMol';
      des      = 'MÓDULO ' + molDesx + ' fue ingresado.';
      idEtaDes = 6;
      lgDes    = 'INGRESO DE MÓDULO';
    }
    this.serviLoad.sumar.emit(2);
    let modulo = {'molDes': molDesx , 'molIcon' : molIcon };
